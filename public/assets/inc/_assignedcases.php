...
if ($_SESSION['logged_in_user'] == 'vendor') {
    ...
    $query = "SELECT DISTINCT test_wf_id, val_wf_id, t1.unit_id, t1.test_id, t1.equip_id, equipment_code, equipment_category, t1.vendor_id, test_wf_current_stage
FROM tbl_test_schedules_tracking t1, equipments t2
WHERE t1.equip_id=t2.equipment_id AND test_wf_current_stage='1' AND vendor_id!=0";
    ...
    if (empty($results)) {
        ...
        foreach ($results as $row) {
            ...
            echo "<td>&nbsp;</td>";
            echo "<td><a href='updatetaskdetails.php?test_id=" . urlencode($row['test_id']) . "&val_wf_id=" . urlencode($row["val_wf_id"]) . "&test_val_wf_id=" . urlencode($row["test_wf_id"]) . "&current_wf_stage=" . urlencode($row["test_wf_current_stage"]) . "&equip_id=" . urlencode($row["equip_id"]) . "' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Start</a> </td>";
            echo "</tr>";
            $count = $count + 1;
        }
    }
    ...
    $query = "SELECT DISTINCT test_wf_id, val_wf_id, t1.unit_id, t1.test_id, t1.equip_id, equipment_code, equipment_category, t1.vendor_id, test_wf_current_stage
FROM tbl_test_schedules_tracking t1, equipments t2
WHERE t1.equip_id=t2.equipment_id AND (test_wf_current_stage='3B' OR test_wf_current_stage='4B')";
    ...
    if (empty($results)) {
      //  echo "<tr><td colspan='7'>Nothing is pending</td></tr>";
        ...
        foreach ($results as $row) {
            ...
            echo "<td>" . htmlspecialchars($row["equipment_code"], ENT_QUOTES, 'UTF-8') . " </td>";
            echo "<td>&nbsp;</td>";
            echo "<td><a href='updatetaskdetails.php?test_id=" . urlencode($row['test_id']) . "&val_wf_id=" . urlencode($row["val_wf_id"]) . "&test_val_wf_id=" . urlencode($row["test_wf_id"]) . "&current_wf_stage=" . urlencode($row["test_wf_current_stage"]) . "' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Start</a> </td>";
            echo "</tr>";
            $count = $count + 1;
        }
    }
...
if ($_SESSION['logged_in_user'] == 'employee' and $_SESSION['department_id'] == 1) {
    ...
    if (empty($results)) {
        ...
        foreach ($results as $row) {
            ...
            echo "<td>";
            if ($row['test_type'] == 'V') {
                echo "<a href='#' data-toggle='modal' data-target='#viewProtocolModal' data-load-url='viewprotocol_modal.php?equipment_id=" . urlencode($row["equip_id"]) . "&val_wf_id=" . urlencode($row["val_wf_id"]) . "' class='btn btn-primary btn-small'  role='button' aria-pressed='true'>View</a>";
            } else {
                echo "-";
            }
            echo "</td>";
            echo "<td><a href='updatetaskdetails.php?test_id=" . $row['test_id'] . "&val_wf_id=" . $row["val_wf_id"] . "&test_val_wf_id=" . $row["test_wf_id"] . "&current_wf_stage=" . $row["test_wf_current_stage"] . "' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Start</a> </td>";
            echo "</tr>";
            $count = $count + 1;
        }
    }
    ...
    <tbody>';
        ...
        if (empty($results)) {
            ...
            foreach ($results as $row) {
                ...
                echo "<td><a href='updateschedulestatus.php?sch_type=" . urlencode($sch_type) . "&schedule_id=" . urlencode($row['schedule_id']) . "&schedule_year=" . urlencode($row["schedule_year"]) . "&unit_id=" . urlencode($row["unit_id"]) . "' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Manage</a> </td>";
    ...
    <tbody>';
        ...
        if (empty($results)) {
            ...
            foreach ($results as $row) {
                ...
                // echo "<td><a href='updateschedulestatus.php?schedule_id=".$row['schedule_id']."&schedule_year=".$row["schedule_year"]."&unit_id=".$row["unit_id"]."' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Manage</a> </td>";
                echo "<td><a href='updateschedulestatus.php?sch_type=" . urlencode($sch_type) . "&schedule_id=" . urlencode($row['schedule_id']) . "&schedule_year=" . urlencode($row["schedule_year"]) . "&unit_id=" . urlencode($row["unit_id"]) . "' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Manage</a> </td>";
