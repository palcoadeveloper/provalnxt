...
$query=" select val_wf_id,t.unit_id, equipment_code, equipment_category, actual_wf_start_datetime from tbl_val_wf_tracking_details t, equipments e where
 t.equipment_id=e.equipment_id and t.val_wf_current_stage='3' and
...
echo "<div class='table-responsive'><table id='datagrid-level2approval' class='table table-sm table-bordered dataTable no-footer text-center'>
                      <thead>
                        <tr>
                          <th> # </th>
...
else
{
    ...
    foreach ($countcompletedtasks as $row) {
        ...
        echo "<td><a href='pendingforlevel2approval.php?approval_stage=2&val_wf_tracking_id=".$val_approval_trk_id."&val_wf_id=".$row["val_wf_id"]."' class='btn btn-primary btn-small' role='button' aria-pressed='true'>Manage</a> </td>";
        echo "</tr>";
        $count=$count+1;
    }
