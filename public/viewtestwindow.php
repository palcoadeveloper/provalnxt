...
if (!$test_val_wf_id || !$test_id || !$current_wf_stage) {
    SecurityUtils::logSecurityEvent('invalid_parameters', 'Missing required parameters for test window', [
  ...
  if(src.indexOf('.pdf')==-1) {
    ...
    $(this).find('.modal-body > img.image_modal').attr('hidden', true);
      ...
      department_id = "<?php if (empty($_SESSION['department_id'])) {
                          ...
                          echo htmlspecialchars($_SESSION['department_id'], ENT_QUOTES, 'UTF-8');
                        } ?>";
      logged_in_user = "<?php echo htmlspecialchars($_SESSION['logged_in_user'], ENT_QUOTES, 'UTF-8'); ?>";
      read_mode = "<?php echo (!empty(secure_get('mode', 'string')) ? 'yes' : 'no') ?>";
