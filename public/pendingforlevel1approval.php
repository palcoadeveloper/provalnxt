...
try {
    ...
    error_log("Error fetching deviation remarks: " . $e->getMessage());
    $deviation_remarks = '';
}
...
try {
    ...
    error_log("Error fetching approver details: " . $e->getMessage());
    $approver_details = [];
}

// Check if user is authorized as a level 1 approver
$is_authorized = false;
if (!empty($approver_details)) {
    $authorized_approvers = [
        $approver_details['level1_approver_engg'] ?? 0,
        $approver_details['level1_approver_hse'] ?? 0,
        $approver_details['level1_approver_qc'] ?? 0,
        $approver_details['level1_approver_qa'] ?? 0,
        $approver_details['level1_approver_user'] ?? 0
    ];
    
    $is_authorized = in_array(intval($_SESSION['user_id']), array_map('intval', $authorized_approvers));
}

if ($is_authorized) {
    try {
        if (intval($_SESSION['department_id']) == 1) { // Engineering
            $query_result = DB::queryFirstField(
                "SELECT level1_eng_approval_by FROM tbl_val_wf_approval_tracking_details WHERE val_wf_id = %s",
                $_GET['val_wf_id']
            );
            
            if (empty($query_result)) {
                echo htmlspecialchars("The case is assigned to you", ENT_QUOTES, 'UTF-8');
            } else {
                echo htmlspecialchars("The case is already approved by the engineering team", ENT_QUOTES, 'UTF-8');
            }
        } else if (intval($_SESSION['department_id']) == 8) { // QA
            $query_result = DB::queryFirstField(
                "SELECT level1_qa_approval_by FROM tbl_val_wf_approval_tracking_details WHERE val_wf_id = %s",
                $_GET['val_wf_id']
            );
            ...
            if (empty($query_result)) {
                ...
                echo htmlspecialchars("The case is already approved by the QA team", ENT_QUOTES, 'UTF-8');
            }
        } else if (intval($_SESSION['department_id']) == 0) { // QC
            $query_result = DB::queryFirstField(
                "SELECT level1_qc_approval_by FROM tbl_val_wf_approval_tracking_details WHERE val_wf_id = %s",
                $_GET['val_wf_id']
            );
            ...
            if (empty($query_result)) {
                ...
                echo htmlspecialchars("The case is already approved by the QC team", ENT_QUOTES, 'UTF-8');
            }
        } else if (intval($_SESSION['department_id']) == 7) { // EHS
            $query_result = DB::queryFirstField(
                "SELECT level1_hse_approval_by FROM tbl_val_wf_approval_tracking_details WHERE val_wf_id = %s",
                $_GET['val_wf_id']
            );
            ...
            if (empty($query_result)) {
                ...
                echo htmlspecialchars("The case is already approved by the EHS team", ENT_QUOTES, 'UTF-8');
            }
        } else {
            $query_result = DB::queryFirstField(
                "SELECT level1_user_dept_approval_by FROM tbl_val_wf_approval_tracking_details WHERE val_wf_id = %s",
                $_GET['val_wf_id']
            );
            ...
            if (empty($query_result)) {
                ...
                echo htmlspecialchars("The case is already approved by the QA/User team", ENT_QUOTES, 'UTF-8');
            }
        }
    } catch (Exception $e) {
        error_log("Error checking approval status: " . $e->getMessage());
        echo htmlspecialchars("Error checking approval status", ENT_QUOTES, 'UTF-8');
    }
} else {
    // User not authorized as level 1 approver - check self-assignment requirements
    try {
        if (intval($_SESSION['department_id']) == 1) { // Engineering
            $query_result = DB::queryFirstField(
                "SELECT level1_eng_approval_by FROM tbl_val_wf_approval_tracking_details WHERE val_wf_id = %s",
            ...
            if (empty($query_result)) {
                echo htmlspecialchars("Self-assignment required", ENT_QUOTES, 'UTF-8');
            } else {
                echo htmlspecialchars("The case is already approved by the engineering team", ENT_QUOTES, 'UTF-8');
            }
        } else if (intval($_SESSION['department_id']) == 8) { // QA
        ...
        error_log("Error checking self-assignment status: " . $e->getMessage());
        echo htmlspecialchars("Error checking assignment status", ENT_QUOTES, 'UTF-8');
    }
}
?>
<!DOCTYPE html>
...
$(document).ready(function(){
    ...
    $('#btnSendBack').on('click', function(e) {
        ...
        if ($('#level1_approver_remark').val().trim().length == 0) {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
        ...
        setSuccessCallback(function(response) {
            ...
            if (response.temp_auth_token) {
                var secureUrl = "core/data/save/sendbackreport.php?wf_id=<?php echo $_GET['val_wf_id']?>&test_wf_id=&level1_approver_remark=" + 
                    encodeURIComponent($("#level1_approver_remark").val()) + 
                    "&deviation_remark=" + encodeURIComponent($("#deviation_remark").val()) + 
                    "&val_wf_approval_tracking_id=<?php echo $_GET['val_wf_tracking_id'] ?>&approval_level=1" +
    ...
    $('#btnSubmit').on('click', function(e) {
        ...
        var form = document.getElementById('formloapproval');
        
        if ((needdeviationremarks == true && $('#deviation_remark').val().trim().length == 0) || 
            $('#level1_approver_remark').val().trim().length == 0) {
        ...
        setSuccessCallback(function(response) {
            ...
            Swal.fire({
                ...
                }).appendTo('#formloapproval');
                
                // Submit the form after modal is closed
                console.log("Submitting form...");
                $('#formloapproval').submit();
            });
        });
    ...
    if(diff >= VALIDATION_DEVIATION_THRESHOLD_DAYS && deviation_remark == '') {
        ...
        $('.dev_remarks').show();
        alert('The validation study was initiated more than ' + VALIDATION_DEVIATION_THRESHOLD_DAYS + ' days ago. Kindly review the deviation remarks.');
    ...
    // Approve button handler is now handled by the sophisticated handler in getuploadedfilesonlevel1.php
    // which includes confirmation dialog and document view validation

    // Reject button handler is now handled by the sophisticated handler in getuploadedfilesonlevel1.php
    // which includes confirmation dialog and document view validation
    ...
    $("#emdlbtnsubmit").on('click', function(e) {
        ...
        if (action === 'sendback') {
            // SEND BACK ACTION
            console.log("Performing send back action");
            $.ajax({
                ...
                data: {
                    ...
                    test_wf_id: '',
                    level1_approver_remark: $("#level1_approver_remark").val(),
                    deviation_remark: $("#deviation_remark").val(),
                    val_wf_approval_tracking_id: "<?php echo $_GET['val_wf_tracking_id'] ?>",
                    approval_level: 1 // Explicitly specify this is a level 1 approval
                },
                dataType: "json",
                success: function(response) {
                        ...
                        <div class="col-lg-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form id="formloapproval" action="core/data/save/savelevel1approvaldata.php" method="get">
    ...
    <input type="hidden" name="val_wf_tracking_id" value="<?php echo $_GET['val_wf_tracking_id']?>"/>
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="3"><?php if($val_wf_details['val_wf_current_stage']==2)
                                                {
                                                    echo 'Pending for Level I approval.';
                                                    }
                                                    ?></td>
                                            </tr>
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="3">
                                                    <a href='#' data-toggle='modal' data-target='#viewProtocolModal' data-load-url='viewprotocol_modal.php?equipment_id=<?php echo $val_wf_details['equipment_id']?>&val_wf_id=<?php echo $_GET['val_wf_id']?>' class='btn btn-primary btn-small'  role='button' aria-pressed='true'>View</a>
                                                </td>
                                            </tr>
                                            ...
                                            <tr class="dev_remarks">
                                                ...
                                                <td colspan="3">
                                                    <input type="text" id="deviation_remark" name="deviation_remark" class="form-control" value='<?php echo (empty($deviation_remarks))?'':$deviation_remarks;?>'readonly/>
                                                </td>
                                            </tr>
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="3">
                                                    <textarea rows="2" id="level1_approver_remark" name="level1_approver_remark" class="form-control"></textarea>
                                            ...
                                            <!-- Replace the existing submit button row with this code -->
<tr>
    <td colspan="4">
        <div class="d-flex justify-content-center">
