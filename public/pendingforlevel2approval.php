...
// Check if user is authorized as unit head for level 2 approval
try {
    if (isset($_SESSION['is_unit_head']) && $_SESSION['is_unit_head'] == "Yes") {
        $result = DB::query(
            "SELECT level3_unit_head_approval_by FROM tbl_val_wf_approval_tracking_details 
             WHERE val_wf_id IN (
                SELECT val_wf_id FROM tbl_val_wf_tracking_details 
                WHERE val_wf_current_stage = 3 AND val_wf_id = %s AND unit_id = %i
             )", 
    ...
    error_log("Error checking level 2 approval authorization: " . $e->getMessage());
...
$(document).ready(function(){
 ...
 // SEND BACK BUTTON HANDLER - FIXED VERSION
$('#btnSendBack').on('click', function(e) {
    ...
    if ($('#level2_approver_remark').val().trim().length == 0) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
        ...
        setSuccessCallback(function(response) {
            ...
            if (response.temp_auth_token) {
                var secureUrl = "core/data/save/sendbackreport.php?wf_id=<?php echo $_GET['val_wf_id']?>&test_wf_id=&level2_approver_remark=" + 
                    encodeURIComponent($("#level2_approver_remark").val()) + 
                    "&deviation_remark=" + encodeURIComponent($("#deviation_remark").val()) + 
                    "&val_wf_approval_tracking_id=<?php echo $_GET['val_wf_tracking_id'] ?>&approval_level=2" +
    ...
    $('#btnSubmit').on('click', function(e) {
        ...
        if ($('#level2_approver_remark').val().trim().length == 0 || (needdeviationremarks == true && $('#deviation_remark').val().trim().length == 0)) {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                        ...
                        <div class="col-lg-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form id="formqaapproval" action="core/data/save/savelevel2approvaldata.php" method="get" class="needs-validation" novalidate>
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="3"><?php 
                                                if($val_wf_details['val_wf_current_stage']==3)
                                                {
                                                    echo "Pending for Level II approval.";
                                                }
                                                ?></td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="3">
                                                    <textarea rows="2" id="level2_approver_remark" name="level2_approver_remark" class="form-control" required></textarea>
