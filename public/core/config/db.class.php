...
// Database constants are defined in config.php and loaded dynamically
...
class DB {
    ...
    // initial connection - initialized dynamically from config constants
    ...
    private static function initializeConfig() {
        if (DB::$dbName === null && defined('DB_NAME')) {
            DB::$dbName = DB_NAME;
            DB::$user = DB_USER;
            DB::$password = DB_PASS;
            DB::$host = DB_HOST;
            DB::$port = defined('DB_PORT') ? DB_PORT : 3306;
        }
    }
    
    public static function getMDB() {
        // Initialize configuration from constants if not already done
        DB::initializeConfig();
...
if (!function_exists('meekrodb_error_handler')) {
