...
{
    $audit_category="('tran_login_int_emp','security_error')";
}
else if($_GET['audittrailcategory']=='tran_login_ext_emp')
{
    $audit_category="('tran_login_ext_emp','security_error')";
}
else if($_GET['audittrailcategory']=='tran_logout')
{
    $audit_category="('tran_logout','security_error')";
}
...
{
    $audit_category="('tran_traindtls_removed', 'tran_rsch_gen', 'tran_rsch_rej_eng', 'tran_rsch_rej_qa', 'tran_rsch_app_eng', 'tran_rsch_app_qa', 'tran_vsch_gen', 'tran_vsch_app_eng', 'tran_vsch_app_qa', 'tran_vsch_rej_eng', 'tran_vsch_rej_qa','tran_vsch_gen_failed','tran_rsch_gen_failed','tran_vsch_approve','tran_rsch_approve', 'tran_view_schedule')";
}
else if($_GET['audittrailcategory']=='tran_valbgn')
{
    $audit_category="('tran_rtbgn', 'tran_valbgn', 'tran_ireview_approve', 'tran_ereview_approve', 'tran_file_upload', 'tran_iupload_files_app', 'tran_eupload_files_app', 'tran_upload_files_rej', 'tran_teamapp_eng', 'tran_teamapp_qa', 'tran_teamapp_ehs', 'tran_teamapp_qc', 'tran_teamapp_user', 'tran_level2app_uh', 'tran_level3app_qh', 'tran_print_report','tran_upload_files_app','tran_file_view','tran_file_view_validation','tran_teamapp_sendback','tran_submitted_approval','add_remarks_success','add_remarks_failed','tran_valadhoc_update','tran_valadhoc_update_failed','tran_rtreq_update','tran_rtreq_add','tran_valadhoc_add')";
}
...
{
    $audit_category="('master_add_eq', 'master_update_eq', 'master_add_etv', 'master_update_etv', 'master_add_test', 'master_update_test', 'master_users_add', 'master_users_addc', 'master_users_updatec', 'master_users_updatev', 'master_add_vendors', 'master_update_vendors','master_users_addv')";
}
...
if(empty($_GET['export']))
{
    ...
    // Set the HTML footer
    // Note: You'll need to define BASE_URL and $requester_details variables
    // For now, using placeholder values - replace these with actual session/user data
   // $base_url = defined('BASE_URL') ? BASE_URL : 'http://your-domain.com/';
   ...
   on u.unit_id=un.unit_id where user_id=".$_SESSION['user_id']);
    ...
    $mpdf->Output('AuditTrailLogReport-' . date('d.m.Y') . '.pdf', 'D');
}
else // Display in browser
{
    echo "<div class='table-responsive'><table id='datagrid-audit-trail' class='table table-bordered'>
    <thead>
    <tr>
    <th> # </th>
    <th> Change Type</th>
    ...
    if(empty($results)) {
        ...
        foreach ($results as $row) {
            ...
            echo "<td>".(!empty($row["user_name"]) ? $row["user_name"] : "-")." </td>";
            echo "<td>".date('d.m.Y H:i:s', strtotime($row["change_datetime"]))." </td>";
            echo "</tr>";
            $count++;   
        }
    }
