<?php
/**
 * Reject Termination Request (QA Head)
 * Rejects termination and moves from 98B to 98E
 */
...
if (!isset($_SESSION['is_qa_head']) || $_SESSION['is_qa_head'] !== 'Yes') {
    ...
    echo json_encode(['status' => 'error', 'message' => 'Access denied. Only QA Head can reject termination requests.']);
    exit();
}
...
try {
    ...
    if ($validation['val_wf_current_stage'] !== '98B') {
        echo json_encode(['status' => 'error', 'message' => 'Invalid workflow stage. Can only reject termination requests in stage 98B.']);
        exit();
    }
    ...
    if (empty($validation['stage_before_termination'])) {
        ...
        error_log("QA rejection: Non-started validation $val_wf_id being archived as $new_val_wf_id");
        ...
        $update_result = DB::update('tbl_val_wf_tracking_details', [
            ...
            'val_wf_current_stage' => '98E', // Mark as rejected by QA
            'stage_assigned_datetime' => date('Y-m-d H:i:s'),
            'last_modified_date_time' => date('Y-m-d H:i:s'),
            'tr_approver_remarks' => $combined_remarks,
        ...
        error_log("QA rejection: Restoring validation $val_wf_id to stage $restored_stage");
    ...
    if ($update_result) {
        ...
        $audit_stage = empty($validation['stage_before_termination']) ? '98E' : $validation['stage_before_termination'];
        DB::insert('audit_trail', [
            ...
            'wf_stage' => $audit_stage // 98E for non-started, or restored stage for started validations
        ]);

        // Log the action
        DB::insert('log', [
            'change_type' => 'termination_rejected_qa',
            'table_name' => 'tbl_val_wf_tracking_details',
            'change_description' => 'Termination rejected by QA Head for workflow: ' . $val_wf_id . '. Reason: ' . substr($rejection_reason, 0, 200),
            'change_by' => $_SESSION['user_id'],
            'unit_id' => $_SESSION['unit_id']
        ]);

        echo json_encode(['status' => 'success', 'message' => 'Termination request rejected by QA Head']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Failed to update workflow stage']);
    }

} catch (Exception $e) {
    error_log("Error rejecting termination by QA: " . $e->getMessage());
