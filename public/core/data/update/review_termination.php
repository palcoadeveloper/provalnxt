<?php
/**
 * Review Termination Request (Engineering Dept Head)
 * Forwards termination request from 98A to 98B
 */
...
if (!isset($_SESSION['is_dept_head']) || $_SESSION['is_dept_head'] !== 'Yes' ||
    ...
    echo json_encode(['status' => 'error', 'message' => 'Access denied. Only Engineering Department Head can review termination requests.']);
    exit();
}
...
if (!isset($_POST['next_stage']) || $_POST['next_stage'] !== '98B') {
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Invalid next stage']);
    exit();
}

// Reviewer remarks from modal (user_remark is the standard modal parameter)
$reviewer_remarks = isset($_POST['user_remark']) ? trim($_POST['user_remark']) : '';
...
try {
    ...
    $validation = DB::queryFirstRow(
        "SELECT val_wf_current_stage, unit_id FROM tbl_val_wf_tracking_details WHERE val_wf_id = %s",
        $val_wf_id
    );
    ...
    if ($validation['val_wf_current_stage'] !== '98A') {
        echo json_encode(['status' => 'error', 'message' => 'Invalid workflow stage. Can only review termination requests in stage 98A.']);
        exit();
    }
    ...
    // Update to stage 98B (Reviewed by Engg Dept Head) with reviewer remarks
    $update_result = DB::update('tbl_val_wf_tracking_details', [
        'val_wf_current_stage' => '98B',
        'stage_assigned_datetime' => date('Y-m-d H:i:s'),
        'last_modified_date_time' => date('Y-m-d H:i:s'),
        'tr_reviewer_remarks' => $reviewer_remarks // Save Engineering Dept Head remarks
    ], 'val_wf_id = %s', $val_wf_id);

    if ($update_result) {
        // Insert audit trail for validation-level stage transition to 98B
        DB::insert('audit_trail', [
            ...
            'wf_stage' => '98B' // Reviewed by Engineering Dept Head
        ]);

        // Log the action
        DB::insert('log', [
            'change_type' => 'termination_reviewed',
            'table_name' => 'tbl_val_wf_tracking_details',
            'change_description' => 'Termination request reviewed and forwarded to QA Head by Engineering Dept Head for workflow: ' . $val_wf_id,
            'change_by' => $_SESSION['user_id'],
            'unit_id' => $_SESSION['unit_id']
        ]);

        echo json_encode(['status' => 'success', 'message' => 'Termination request reviewed and forwarded to QA Head']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Failed to update workflow stage']);
    }

} catch (Exception $e) {
    error_log("Error reviewing termination: " . $e->getMessage());
