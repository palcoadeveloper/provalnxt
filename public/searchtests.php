...
<html lang="en">
  <head>
    ...
    function restoreSearchState() {
        ...
        if (restoreFlag === '1') {
            console.log('Restoring test search state from URL parameters');

            // Restore form values
            const testPerformedBy = getUrlParameter('test_performed_by');
            const testStatus = getUrlParameter('test_status');

            if (testPerformedBy && testPerformedBy !== '') {
                $('#test_performed_by').val(testPerformedBy);
            }
            ...
            setTimeout(function() {
                console.log('Auto-submitting restored test search');

                // Show a brief notification that search is being restored
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        title: 'Restoring Search Results',
                        text: 'Taking you back to your previous test search...',
                        icon: 'info',
                        timer: 1500,
...
e.preventDefault();
		...
		$('#pleasewaitmodal').modal('show');
                     ...
                     setTimeout(function() {
                         ...
                         // Smooth scroll to results section when coming back from test details
                         const restoreFlag = getUrlParameter('restore_search');
                         if (restoreFlag === '1') {
                             setTimeout(function() {
